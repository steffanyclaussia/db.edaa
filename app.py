...
def parse_harga_beras(file_like):
    ...
    # Deteksi tahun dari teks sekitar header
    tahun = 2024
...
# ==========================================
# 3. STREAMLIT UI (TYPOGRAPHY & DESIGN)
# ==========================================
st.set_page_config(page_title="Analisis Harga Beras RCBD", layout="wide")

st.markdown(f"""
<style>
    html, body, [class*="st-"], .stMarkdown, .stTable, .stDataFrame, div[data-testid="stMetricValue"] {{
    ...
    div[data-testid="stMetricValue"] {{ font-size: 2.2rem !important; font-weight: bold !important; white-space: nowrap !important; }}
    .info-card {{
...
st.markdown(f'<div class="header-box"><h1 style="margin:0; font-size: 3rem;">Laporan Analisis Harga Beras</h1></div>', unsafe_allow_html=True)
...
if up:
    ...
    with t1:
        fig_line = px.line(df_long, x="bulan", y="harga", color="kualitas", color_discrete_map=QUAL_COLORS, markers=True, title="Tren Fluktuasi Harga")
        fig_line.update_layout(font_family="Times New Roman")
        st.plotly_chart(fig_line, use_container_width=True)
        
        c_v1, c_v2 = st.columns(2)
        with c_v1:
            fig_box = px.box(df_long, x="kualitas", y="harga", color="kualitas", color_discrete_map=QUAL_COLORS, title="Sebaran Harga")
            fig_box.update_layout(font_family="Times New Roman")
            st.plotly_chart(fig_box, use_container_width=True)
        with c_v2:
            avg_data = df_long.groupby("kualitas")['harga'].mean().reset_index()
            fig_bar = px.bar(avg_data, x="kualitas", y="harga", color="kualitas", color_discrete_map=QUAL_COLORS, title="Rata-rata Tahunan")
            fig_bar.update_layout(font_family="Times New Roman")
            st.plotly_chart(fig_bar, use_container_width=True)

    with t2:
        st.markdown("### 📋 Matriks Harga Beras")
        # PERBAIKAN: Penanganan error jika Matplotlib gagal dimuat untuk gradien
        try:
            ...
            st.warning("Pustaka Matplotlib tidak tersedia. Gradien warna tabel dinonaktifkan.")

    with t3:
        st.markdown("### 🔬 Analisis Inferensial (ANOVA RCBD)")
        ...
        for label, hyp in hypotheses:
            ...
            # Penggunaan .item() untuk ekstraksi skalar yang aman
            ph_results.append({
                "Perbandingan": label, 
    ...
    st.info("👋 Selamat Datang. Silakan unggah file CSV hasil laporan BPS untuk memulai analisis.")
